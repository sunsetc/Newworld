...
<div class="notepage">
    ...
    <select class="qnaok">
        ...
        <option value="ok">항공</option>
    </select>
    <select class="qnatitle">
        <option value="allcheck" selected>전체</option>
        <option value="bnTitle">제목</option>
        <option value="bnContent">내용</option>
    </select>
    ...
    <button class="qnabtn" onclick="getBoardNote()">검색</button>
    </center>
				...
				case '전체' :
					{
						switch(qnatitleSel){
						case 'allcheck':
							url = "/newworlds-note?bnCategory=&bnTitle=" + document.querySelector('.qnasearch').value + "&bnContent=" + document.querySelector('.qnasearch').value;
						break;
						
						case 'bnTitle': 
							url = "/newworlds-note?bnCategory=&bnTitle=" + document.querySelector('.qnasearch').value;
						break;
						
						case 'bnContent': 
							url = "/newworlds-note?bnCategory=&bnContent=" + document.querySelector('.qnasearch').value;
						break;
						}
					} 
					break;
				default : 
					{
						switch(qnatitleSel){
						case 'allcheck':
							url = "/newworlds-note?bnCategory=" + qnaokSel + "&bnTitle=" + document.querySelector('.qnasearch').value + "&bnContent=" + document.querySelector('.qnasearch').value;
						break;
						
						case 'bnTitle' : 
							url = "/newworlds-note?bnCategory=" + qnaokSel + "&bnTitle=" + document.querySelector('.qnasearch').value;
						break;
						
						case 'bnContent' : 
							url = "/newworlds-note?bnCategory=" + qnaokSel + "&bnContent=" + document.querySelector('.qnasearch').value;
						break;
						}
					}
					break;
...
/* function getBoardNote(){  //공지사항 목록&검색
	const qnaok = document.querySelector('select.qnaok');
	const qnaokSel = qnaok.options[qnaok.selectedIndex].innerHTML;
	const qnatitle = document.querySelector('select.qnatitle');
	const qnatitleSel = qnatitle.options[qnatitle.selectedIndex].value;
	if(qnaokSel == '전체' && qnatitleSel == 'allcheck'){
		fetch('/newworlds-note?bnCategory=&bnTitle='+ document.querySelector('.qnasearch').value + '&bnContent=' + document.querySelector('.qnasearch').value)
		.then(function(res){
			return res.json();
		})
	...
	}else if(qnaokSel == '전체' && qnatitleSel == 'bnTitle'){
		fetch('/newworlds-note?bnCategory=&bnTitle='+ document.querySelector('.qnasearch').value)
		.then(function(res){
			return res.json();
		})
	...
	}else if(qnaokSel == '전체' && qnatitleSel == 'bnContent'){
		fetch('/newworlds-note?bnCategory=&bnContent='+ document.querySelector('.qnasearch').value)
		.then(function(res){
			return res.json();
		})
		...
		fetch('/newworlds-note?bnCategory=' + qnaokSel + '&' + qnatitleSel + '=' + document.querySelector('.qnasearch').value)
		.then(function(res){
			return res.json();
		})
